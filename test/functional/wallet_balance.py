...
class WalletTest(BWScoinTestFramework):
    ...
    def run_test(self):
        ...
        self.sync_all()
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), 1510, [Decimal('0.01'), Decimal('0.02')])
        ...
        assert_equal(self.nodes[1].getbalance(), Decimal('1479.99'))  # change from node 1's send

        # NOTE: Skip below test (minconf=0) until newer getbalance implementation that doesn't use CWallet::GetLegacyBalance() gets merged in
        # Same with minconf=0
        # assert_equal(self.nodes[0].getbalance(account='*', minconf=0), Decimal('1519.99'))
        # assert_equal(self.nodes[1].getbalance(account='*', minconf=0), Decimal('1479.99'))

        # NOTE: Skip below test (minconf=0) until newer getbalance implementation that doesn't use CWallet::GetLegacyBalance() gets merged in
        # getbalance with a minconf incorrectly excludes coins that have been spent more recently than the minconf blocks ago
        # TODO: fix getbalance tracking of coin spentness depth
        # assert_equal(self.nodes[0].getbalance(account='*', minconf=1), Decimal('9.99'))
        # assert_equal(self.nodes[1].getbalance(account='*', minconf=1), Decimal('-10.01'))

        # getunconfirmedbalance
        assert_equal(self.nodes[0].getunconfirmedbalance(), Decimal('1510'))  # output of node 1's spend
        ...
        assert_equal(self.nodes[0].getwalletinfo()["unconfirmed_balance"], Decimal('1510'))  # output of node 1's send
        assert_equal(self.nodes[0].getunconfirmedbalance(), Decimal('1510'))
        ...
        assert_equal(self.nodes[0].getbalance(), Decimal('1519.99'))  # node 1's send plus change from node 0's send
        assert_equal(self.nodes[1].getbalance(), Decimal('1479.98'))  # change from node 0's send

        # Send total balance away from node 1
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), Decimal('1479.97'), [Decimal('0.01')])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])
        self.nodes[1].generatetoaddress(2, RANDOM_COINBASE_ADDRESS)
        self.sync_all()

        # NOTE: Remove deprecated account='*' when a newer getbalance implementation that handles '*' as a dummy first argument gets merged in
        # getbalance with a minconf incorrectly excludes coins that have been spent more recently than the minconf blocks ago
        # TODO: fix getbalance tracking of coin spentness depth
        # getbalance with minconf=3 should still show the old balance
        assert_equal(self.nodes[1].getbalance(account='*', minconf=3), Decimal('0'))

        # getbalance with minconf=2 will show the new balance.
        assert_equal(self.nodes[1].getbalance(account='*', minconf=2), Decimal('0'))
