...
{
    return "    \"type\" : txtype,        (string) indicates what type the transaction has: regular or one of the stake types (vote, purchase, revocation)"
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error{
            ...
            "     \"addresses\" : [          (array of string) array of bwscoin addresses\n"
...
UniValue txfeeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 3)
        throw std::runtime_error{
            ...
            "  },\n"
            "  \"feeinfoblocks\":  Ticket fee information for a given list of blocks descending from the chain tip (units: BWScoin/kB)\n"
            "  [\n"
            "       {\n"
            ...
            "       },\n"
            "  ],\n"
            "  \"feeinforange\":  Ticket fee information for a window period where the stake difficulty was the same (units: BWScoin/kB)\n"
            "  {\n"
