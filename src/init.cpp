 ...
 * between bwscoind, and bwscoin-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), BWSCOIN_CONF_FILENAME));
#ifdef USE_CHAINPARAMS_CONF
    strUsage += HelpMessageOpt("-chainparams-conf=<file>", strprintf(_("Specify chainparams configuration file (default: %s)"), BWSCOIN_CHAINPARAMS_CONF_FILENAME));
    ...
    strUsage += HelpMessageOpt("-maxdepthfornotification", strprintf(_("The maximum depth of the blockchain fork on which a block is in order to be notified to the node and peers. (default: %u)"), DEFAULT_MAX_DEPTH_FOR_NOTIFICATION));
    if (showDebug) {
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), BWSCOIN_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-structdatacarriersize", strprintf(_("Maximum size of data in structured data carrier transactions we relay and mine (default: %u)"), MAX_OP_STRUCT_RELAY));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/valdi-labs/bwscoin>";
    const std::string URL_WEBSITE = "<https://valdi.ai>";
...
/** Sanity checks
 *  Ensure that BWS Coin is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    nMaxStructDatacarrierBytes = gArgs.GetArg("-structdatacarriersize", nMaxStructDatacarrierBytes);
    ...
    nMaxDepthForNotification = gArgs.GetArg("-maxdepthfornotification", DEFAULT_MAX_DEPTH_FOR_NOTIFICATION);
    if (nMaxDepthForNotification <= 0)
        return InitError("max depth for notification must be positive");
    else
        LogPrintf("Max depth for notification: %d\n", nMaxDepthForNotification);
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single BWS Coin process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", BWSCOIN_CONF_FILENAME)).string());
